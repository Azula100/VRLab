...
const CABLE_RULES = { 'router-switch':'straight', 'pc1-switch':'straight', 'pc2-switch':'straight', 'pc1-pc2':'crossover', 'switch-switch':'crossover' };
...
const state = {
  ...
  powerLight:null, ceilingLights:{light1:null,light2:null}, lightStates:{light1:true,light2:true},
  vrCableHeld:false, vrCableStart:null, vrTempCable:null,
  vrHoveredNode:null, vrFirstNode:null,
  moveSpeed:0.03, playerRig:null,
  correctConns:0, wrongAttempts:0, vrSessionEnded:false,
};

const roomW=10, roomH=5, roomD=10;
const mkMat = (color, rough=0.8, metal=0) => new THREE.MeshStandardMaterial({color, roughness:rough, metalness:metal});

const room3Group = new THREE.Group();
scene.add(room3Group);

// ── FLOOR ──
const floor = new THREE.Mesh(new THREE.PlaneGeometry(roomW,roomD), mkMat(0x8b7355,0.9));
floor.rotation.x = -Math.PI/2; floor.receiveShadow=true; room3Group.add(floor);
[-4,-3,-2,-1,0,1,2,3,4].forEach(i=>{
  const g1=new THREE.Mesh(new THREE.PlaneGeometry(0.02,roomD),new THREE.MeshStandardMaterial({color:0x6b5535}));
  g1.rotation.x=-Math.PI/2; g1.position.set(i,0.001,0); room3Group.add(g1);
  const g2=new THREE.Mesh(new THREE.PlaneGeometry(roomW,0.02),new THREE.MeshStandardMaterial({color:0x6b5535}));
  g2.rotation.x=-Math.PI/2; g2.position.set(0,0.001,i); room3Group.add(g2);
});

// ── WALLS / CEILING ──
const wallMat = mkMat(0xd4c9b0,0.9);
[
  {pos:[0,roomH/2,-roomD/2],rot:[0,0,0]}, {pos:[0,roomH/2,roomD/2],rot:[0,Math.PI,0]},
  {pos:[-roomW/2,roomH/2,0],rot:[0,Math.PI/2,0]}, {pos:[roomW/2,roomH/2,0],rot:[0,-Math.PI/2,0]},
].forEach(({pos,rot})=>{
  const w=new THREE.Mesh(new THREE.PlaneGeometry(roomW,roomH),wallMat.clone());
  w.position.set(...pos); w.rotation.set(...rot); w.receiveShadow=true; room3Group.add(w);
});
const ceiling=new THREE.Mesh(new THREE.PlaneGeometry(roomW,roomD),mkMat(0xf0ece0,0.9));
ceiling.rotation.x=Math.PI/2; ceiling.position.set(0,roomH,0); room3Group.add(ceiling);

// ── CORNICES + BASEBOARDS ──
const corniceH=0.12, corniceD=0.1, baseH=0.1, baseD=0.06;
const corniceMat=mkMat(0xe8e0d0,0.7), baseMat=mkMat(0xc8b89a,0.7);
[
  {pos:[0,roomH-corniceH/2,-roomD/2+corniceD/2],w:roomW,mat:corniceMat},
  {pos:[0,roomH-corniceH/2, roomD/2-corniceD/2],w:roomW,mat:corniceMat,ry:Math.PI},
  {pos:[-roomW/2+corniceD/2,roomH-corniceH/2,0],w:roomD,mat:corniceMat,ry:Math.PI/2},
  {pos:[ roomW/2-corniceD/2,roomH-corniceH/2,0],w:roomD,mat:corniceMat,ry:-Math.PI/2},
  {pos:[0,baseH/2,-roomD/2+baseD/2],w:roomW,h:baseH,d:baseD,mat:baseMat},
  {pos:[0,baseH/2, roomD/2-baseD/2],w:roomW,h:baseH,d:baseD,mat:baseMat,ry:Math.PI},
  {pos:[-roomW/2+baseD/2,baseH/2,0],w:roomD,h:baseH,d:baseD,mat:baseMat,ry:Math.PI/2},
  {pos:[ roomW/2-baseD/2,baseH/2,0],w:roomD,h:baseH,d:baseD,mat:baseMat,ry:-Math.PI/2},
].forEach(({pos,w,h=corniceH,d=corniceD,mat,ry=0})=>{
  const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);
  m.position.set(...pos); m.rotation.y=ry; room3Group.add(m);
});

// ── DOOR ──
const doorGroup=new THREE.Group();
const frameMat=mkMat(0x5c3d1e,0.6);
[[0.05,1.05,0,0.1,2.1,0.15],[-0.9,1.05,0,0.1,2.1,0.15],[-0.425,2.15,0,0.95,0.1,0.15]].forEach(([x,y,z,w,h,d])=>{
  const f=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),frameMat); f.position.set(x,y,z); doorGroup.add(f);
});
const doorPanel=new THREE.Mesh(new THREE.BoxGeometry(0.85,2.0,0.07),mkMat(0x8B5E3C,0.5));
doorPanel.position.set(-0.425,1.0,0); doorGroup.add(doorPanel);
[[0.045,1.5],[0.045,0.5]].forEach(([pz,py])=>{
  const d=new THREE.Mesh(new THREE.BoxGeometry(0.7,0.8,0.02),mkMat(0x7a5230,0.5));
  d.position.set(-0.425,py,pz); doorGroup.add(d);
});
...
doorGroup.position.set(-roomW/2+0.08,0,-1.0); doorGroup.rotation.y=Math.PI/2;
doorGroup.userData.kind='backDoor'; room3Group.add(doorGroup);
...
function drawBoardIdle(){
  ...
  c.clearRect(0,0,w,h);
  c.fillStyle='#0f172a'; roundRect(c,0,0,w,h,24); c.fill();
  c.strokeStyle='rgba(0,212,255,0.25)'; c.lineWidth=4; roundRect(c,2,2,w-4,h-4,22); c.stroke();
  c.fillStyle='#334155'; c.font='bold 36px Segoe UI, Arial';
...
frameB.position.z=-0.022; boardGroup.add(frameB);
...
const hoverSphere=new THREE.Mesh(new THREE.SphereGeometry(0.13,16,16),new THREE.MeshBasicMaterial({color:0x00ffcc,transparent:true,opacity:0.35,wireframe:true}));
hoverSphere.visible=false; room3Group.add(hoverSphere);

const firstNodeMarker=new THREE.Mesh(new THREE.SphereGeometry(0.1,16,16),new THREE.MeshBasicMaterial({color:0xffaa00,transparent:true,opacity:0.6,wireframe:true}));
...
renderer.xr.addEventListener('sessionstart',()=>{
  ...
  setStatus('VR горимд орлоо! A=Router B=Reset X=Straight Y=Crossover','success');
});
...
function showVRResult(){
  ...
  overlay.style.display='flex'; overlay.style.opacity='0'; overlay.style.transform='scale(0.88)';
...
window.hideVRResult=()=>{ const o=document.getElementById('vrResultOverlay'); if(o) o.style.display='none'; };
window.exitVR=()=>{ const s=renderer.xr.getSession(); if(s) s.end(); else setStatus('VR session идэвхгүй.','error'); };
...
function validateAndConnect(connIndex){
  ...
  const required=CABLE_RULES[CONN_PAIRS[connIndex]];
  ...
  if(chosen===required){
    ...
    updateScoreBoard();
    Object.values(cableObjects).forEach(c=>{if(c){const o=c.material.color.clone();c.material.color.setHex(0xff0000);setTimeout(()=>{if(c.material)c.material.color.copy(o);},400);}});
  }
...
function drawCable(key,cableType){
  ...
  const pairs={rs:['router','switch1'],sp1:['switch1','pc1'],sp2:['switch1','pc2'],pp:['pc1','pc2']};
  const [a,b]=pairs[key];
  const start=p[a].clone(), end=p[b].clone();
...
function makeFloatingLabel(text,color,position){
  ...
  ctx.fillStyle=hex; ctx.font='bold 28px Segoe UI,Arial'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(text,128,34);
...
function resetCables(){
  ...
  document.querySelectorAll('.connBtn').forEach(b=>b.classList.remove('active'));
  document.getElementById('resultMsg') && (document.getElementById('resultMsg').innerHTML='');
  showDashedCable(); updateStatus(); updateScoreBoard(); setStatus('Кабелиуд арилгагдлаа.','');
}
...
function onVRTrigger(){
  ...
  if(state.vrFirstNode){
    const sorted=[state.vrFirstNode,nodeName].sort().join('-');
    const connIdx={'router-switch':1,'pc1-switch':2,'pc2-switch':3,'pc1-pc2':4}[sorted]||null;
    ...
    else setStatus(`❌ "${state.vrFirstNode}" ↔ "${nodeName}" холболт тодорхойгүй`,'error');
...
function onVRGripDown(){
  ...
  state.vrFirstNode=null; firstNodeMarker.visible=false; rayMat.color.setHex(0xffaa00);
}
function onVRGripUp(){
  ...
  const sorted=[state.vrCableStart,hit.object.userData.nodeName].sort().join('-');
  const connIdx={'router-switch':1,'pc1-switch':2,'pc2-switch':3,'pc1-pc2':4}[sorted]||null;
  if(connIdx){ selectConn(connIdx); validateAndConnect(connIdx); vrFlash(hit.point,state.selectedCableType==='straight'?0x4ade80:0xf97316); }
  else setStatus(`❌ "${state.vrCableStart}" → "${hit.object.userData.nodeName}" холболт байхгүй`,'error');
  state.vrCableStart=null;
}

const vrBtns={A:false,B:false,X:false,Y:false,MENU:false};
function handleVRGamepad(){
  ...
  session.inputSources.forEach(src=>{
    ...
    if(src.handedness==='right'){
      ...
      if(!vrBtns.MENU&&gp.buttons[3]?.pressed){vrBtns.MENU=true;window.exitVR();}else if(!gp.buttons[3]?.pressed)vrBtns.MENU=false;
    }
...
function updateVRHover(){
  ...
  const activeNode=state.vrCableHeld?state.vrCableStart:(state.vrFirstNode||null);
  if(activeNode){
    if(state.vrTempCable) room3Group.remove(state.vrTempCable);
    const sp=state.positions[nodeToKey[activeNode]];
...
function vrPulse(nodeName){
  ...
  let t=0; const anim=()=>{t+=0.04;ring.scale.setScalar(1+t*2);ring.material.opacity=Math.max(0,0.8-t);if(t<1)requestAnimationFrame(anim);else room3Group.remove(ring);}; anim();
}
...
function makeCableMesh(start,end,color){
  ...
  const mesh=new THREE.Mesh(
    new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts),40,0.016,8,false),
    new THREE.MeshStandardMaterial({color,emissive:0x000000,emissiveIntensity:0})
  );
  room3Group.add(mesh); return mesh;
}
...
function showDashedCable(){dashedState.segs.forEach(s=>{s.dash.visible=true;s.fill.visible=false;});dashedState.fillT=0;dashedState.filling=false;dashedState.done=false;}
function startDashedFill(){if(state.cables.rs)return;dashedState.fillT=0;dashedState.filling=true;dashedState.done=false;}
function stopDashedFill(){dashedState.filling=false;dashedState.done=false;dashedState.fillT=0;dashedState.segs.forEach(s=>s.fill.visible=false);}
function updateDashedCable(delta){
  ...
  if(dashedState.done){
    ...
    dashedState.segs.forEach(s=>{s.fill.visible=true;s.fill.material.opacity=0.5+p*0.5;}); return;
  }
...
const wifiRings=[]; let wifiPhase=0, phoneObj=null, wifiLineMesh=null;
...
function createPhone(rPos){
  ...
  g.add(Object.assign(new THREE.Mesh(new THREE.BoxGeometry(0.08,0.16,0.012),new THREE.MeshStandardMaterial({color:0x111111,roughness:0.3,metalness:0.8}))));
...
function createWifiLine(fromPos,toPos){
  ...
  const geo=new THREE.BufferGeometry().setFromPoints([fromPos.clone().add(new THREE.Vector3(0,0.3,0)),toPos.clone()]);
  wifiLineMesh=new THREE.Line(geo,new THREE.LineDashedMaterial({color:0x00d4ff,dashSize:0.08,gapSize:0.05,transparent:true,opacity:0.8}));
  wifiLineMesh.computeLineDistances(); room3Group.add(wifiLineMesh);
}
...
function updateWifiLine(){
  if(!wifiLineMesh||!phoneObj||!state.positions.router) return;
  const pts=[state.positions.router.clone().add(new THREE.Vector3(0,0.3,0)),phoneObj.position.clone()];
  wifiLineMesh.geometry.dispose(); wifiLineMesh.geometry=new THREE.BufferGeometry().setFromPoints(pts); wifiLineMesh.computeLineDistances();
}

// ── CEILING LIGHTS ──
function addCeilingLight(x,z){
  const group=new THREE.Group();
  const box=new THREE.Mesh(new THREE.BoxGeometry(0.6,0.08,0.6),mkMat(0xeeeeee,0.4)); box.castShadow=true; group.add(box);
  const panel=new THREE.Mesh(new THREE.BoxGeometry(0.52,0.02,0.52),new THREE.MeshStandardMaterial({color:0xfffde7,emissive:0xfffde7,emissiveIntensity:3.0}));
  panel.position.y=-0.05; group.add(panel);
  const wire=new THREE.Mesh(new THREE.CylinderGeometry(0.008,0.008,0.25,6),mkMat(0x333333,0.5)); wire.position.y=0.165; group.add(wire);
  group.position.set(x,roomH-0.04,z); room3Group.add(group);
  const light=new THREE.PointLight(0xfffde7,2.5,8); light.position.set(x,roomH-0.2,z); light.castShadow=true;
  light.shadow.mapSize.set(512,512); room3Group.add(light);
  return {group,light,bulbMat:panel.material,cone:null};
}
state.ceilingLights.light1=addCeilingLight(-2,-1);
state.ceilingLights.light2=addCeilingLight(2,-1);

function toggleLight(idx){
  const l=state.ceilingLights[`light${idx}`]; if(!l) return;
  const on=state.lightStates[`light${idx}`]=!state.lightStates[`light${idx}`];
  l.bulbMat.color.setHex(on?0xffdd88:0x888888); l.bulbMat.emissive.setHex(on?0x442200:0x000000);
...
function togglePower(){
  ...
  if(state.routerOn){
    ...
    if(ind){ind.material.color.setHex(0x00ff44);ind.material.emissive?.setHex(0x00ff44);}
    ...
    document.getElementById('powerStatus') && (document.getElementById('powerStatus').textContent='Router: Асаалттай ✅');
  } else {
    ...
    document.getElementById('powerStatus') && (document.getElementById('powerStatus').textContent='Router: Унтарсан');
    document.getElementById('dataStatus') && (document.getElementById('dataStatus').textContent='Өгөгдөл: Зогссон');
  }
}
...
function addIndicator(pos,name){
  const l=new THREE.Mesh(new THREE.SphereGeometry(0.04,8,8),new THREE.MeshStandardMaterial({color:0x444444,emissive:0x000000,emissiveIntensity:0}));
...
function spawnPacket(from,to,color){
  ...
  room3Group.add(m); state.packets.push({mesh:m,from:from.clone(),to:to.clone(),t:0,speed:0.008+Math.random()*0.004});
}
...
function spawnNetworkPackets(){
  ...
  if(state.cables.rs&&state.cables.sp1&&state.cables.sp2&&p.router&&p.switch1&&p.pc1&&p.pc2){
...
function checkAllConnected(){
  ...
  if(allDone){
    ...
    document.getElementById('dataStatus') && (document.getElementById('dataStatus').textContent='Өгөгдөл: Идэвхтэй 🟢');
    ...
    document.getElementById('resultMsg') && (document.getElementById('resultMsg').innerHTML='<span style="color:#00ff88">🏆 Бүх холболт амжилттай!</span>');
  } else if(state.cables.rs&&state.cables.sp1&&state.cables.sp2&&state.routerOn){
    document.getElementById('dataStatus') && (document.getElementById('dataStatus').textContent='Өгөгдөл: Хэсэгчлэн 🟡');
  }
}
function updateScoreBoard(){
  document.getElementById('correctCount') && (document.getElementById('correctCount').textContent=state.correctConns);
  document.getElementById('wrongCount') && (document.getElementById('wrongCount').textContent=state.wrongAttempts);
}
function updateStatus(){
  const c=Object.values(state.cables).filter(Boolean).length;
  document.getElementById('cableStatus') && (document.getElementById('cableStatus').textContent=`🔌 Холболт: ${c}/${TOTAL_CONNS}`);
}
...
loader.load('./lab4.glb',gltf=>{
  ...
  const router=model.getObjectByName('router'), switch1=model.getObjectByName('switch');
  const pc1=model.getObjectByName('pc1'), pc2=model.getObjectByName('pc2');
  const monitor1=model.getObjectByName('monitor1'), monitor2=model.getObjectByName('monitor2');
  if(!router||!switch1||!pc1||!pc2){setStatus('GLB: router/switch/pc1/pc2 нэр олдсонгүй','error');return;}
  router.traverse(o=>{if(o.isMesh){o.material=o.material.clone();o.material.color.setHex(0x1a3a5c);o.material.emissive?.setHex(0x051525);o.material.roughness=0.4;o.material.metalness=0.7;}});
  const screenTex=new THREE.TextureLoader().load('screen2.jpg');
  [monitor1,monitor2].forEach(m=>{if(!m)return;m.traverse(o=>{if(o.isMesh){o.material=o.material.clone();o.material.map=screenTex;o.material.emissiveMap=screenTex;o.material.emissive.set(0xffffff);o.material.emissiveIntensity=1;}});});
  const sandalColor=new THREE.Color(0x8B5A2B), shireeColor=new THREE.Color(0xD2B48C);
  ['sandal1','sandal2'].forEach(n=>{const obj=model.getObjectByName(n);if(obj)obj.traverse(c=>{if(c.isMesh)c.material=new THREE.MeshStandardMaterial({color:sandalColor,roughness:0.6,metalness:0.2});});});
  const shiree1=model.getObjectByName('shiree1');
  if(shiree1) shiree1.traverse(c=>{if(c.isMesh)c.material=new THREE.MeshStandardMaterial({color:shireeColor,roughness:0.6,metalness:0.2});});
  ...
  buildDashedCable(r,new THREE.Vector3(r.x+0.5,r.y+0.15,r.z+0.3));
  createWifiRings(r); phoneObj=createPhone(r); createWifiLine(r,phoneObj.position);
...
const setStatus=(msg,type='')=>{const el=document.getElementById('status');if(el){el.textContent=msg;el.className=type;}};

// ── UPDATE ──
let glowPhase=0;
room3Group.userData.update=(delta,playerRig)=>{
  if(playerRig) state.playerRig=playerRig;
  if(renderer.xr.isPresenting){handleVRGamepad();updateVRHover();}
  else{
    hoverSphere.visible=false;
  ...
  updatePackets(); updateWifi(delta); updateWifiLine(); updateDashedCable(delta);
